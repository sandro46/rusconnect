...
$companySource = $root->getCompanyInfo(8);
print_r($companySource);
die();
$html = $core->tpl->get('form.schet_na_oplatu.html'); // счет на оплату
//$html = $core->tpl->get('form.akt.html'); // акт по счету


$core->lib->load('topdf');
$pdf = new topdf();
$file = $pdf->add($html, 0, $core->site_id, 'Счет на оплату_'.rand(1, 999).'.pdf');

echo '<script type="text/javascript">document.location.href="'.$file[0].'";</script>';
die();


// Controller logic, source code

//$root->update_bik($core->CONFIG['var_path'].'bnkseek.txt');
