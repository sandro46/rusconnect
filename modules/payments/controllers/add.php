...
$controller->load('admin.payments.php');
$controller->load('platron.php');
$root = new admin_payments($core->CONFIG['pay_handlers']['merchant_id'], $core->CONFIG['pay_handlers']['secret']);

$result = $root->simplePay();

if(!is_array($result))
{
   	$core->title = '';
   	$controller->content = '<h2>Обнаружена ошибка при проведении платежа.</h2> Подробности:<br><div style="border:solid red 1px; padding:8px; font-size:10px;">'.$result.'</div><br>Если Вы уверенны в том, что эта ошибка произошла по нашей вине, пожалуйста, обратитесь в отдел технической поддержки.<br>тел. 8-499-755-95-44'.
	$controller->content .= '<br><a href="'.$root->url['user_failure_url'].'">Вернуться на страницу платежей</a>';
}
else 
	{
