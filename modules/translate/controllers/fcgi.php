...
$services = array('phoneparser', 'scheduler', 'statistic', 'status');

if(!isset($_GET['service']) || !in_array($_GET['service'], $services)) showErrorJson('Service not found.', 1);
if(!isset($_POST['query'])) showErrorJson('Query is empty.', 3);


if($_GET['service'] == 'phoneparser') {
	$controller->load('remoteservice.php');
	$rs = new remoteservice();
	echo json_encode($rs->getPhoneInfo($_POST['query']));
	die();
}

function showErrorJson($text, $errornum) {
	echo json_encode(array('error'=>2, "message"=>$text, 'error_code'=>$errornum)); 
	die();
}

showErrorJson('What you need here?', 3);