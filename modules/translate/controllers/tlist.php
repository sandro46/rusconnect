...
$controller->load('translate.php');
$api = new translate();

if($api->clientId == 1) {
	$controller->tpl = 'full-admin.list.html';
} else {
	$controller->tpl = 'full.list.html';
}

$core->ajax->register($api, 'getTranlateList');
$core->ajax->register($api, 'getFilterData');
$core->ajax->register($api, 'getUserStat');
$core->ajax->register($api, 'getTranslateInfo');
$core->ajax->register($api, 'deleteTranslate');
$core->ajax->register($api, 'getCategoryText');
$core->ajax->register($api, 'addTextToTranslate');
$core->ajax->listen();

$core->tpl->assign('fulllist',$api->getCategoryList());
$core->tpl->assign('fulllistTranslated',json_encode($api->getTranslatedLinks()));
$core->tpl->assign('subtitle', 'Переводы');
