...
{
	if($_GET['step'] == 1) $step = 1;
	if($_GET['step'] == 2) $step = 2;	
}

$album = new album();

if($step == 0) $album->create() or die($album->error);
if($step == 1) $album->postCreate() or die($album->error);
if($step == 2) $album->update();


if($step == 0) $controller->redirect("/".$core->CONFIG['lang']['name']."/albums/edit/id/{$album->id}/new/1/", true);
if($step == 1) $controller->redirect("/".$core->CONFIG['lang']['name']."/albums/list/", true);
if($step == 2) $controller->redirect("/".$core->CONFIG['lang']['name']."/albums/list/", true);
