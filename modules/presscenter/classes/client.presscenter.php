...
class client_presscenter
{
	...
	public function approveActions($id, $action){
		global $core;
		
		if(!intval($id) || $core->user->id != 1)
			return;
		if($action == "ok"){
			$sql = "UPDATE press_posts SET confirmed = 1 WHERE id = {$id}";
			$core->db->query($sql);
		}
		
		if($action == "cancel"){
			$sql = "DELETE FROM press_posts WHERE id = {$id}";
			$core->db->query($sql);
		}
	...
	public function get_topic_list()
	{
		...
		if(isset($_GET['public']) && $core->user->id == 2)
			$sql .= " AND p.user_id > 2 AND p.confirmed = 1";
			
		elseif($core->user->id == 1 && isset($_GET['public']) )
			$sql .= " AND p.user_id > 2 AND p.confirmed = 0";
		elseif($core->user->id == 2)
			$sql .= " AND p.user_id = 1";
		else
			$sql .= " AND p.user_id = ".$core->user->id;
