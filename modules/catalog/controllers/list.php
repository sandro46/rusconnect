...
if($core->user->id != 1) {
	...
	$sql = "SELECT 
					...
					FROM 
								mcms_sites s 
					WHERE 
								s.id = c.site_id LIMIT 1) domain 
			FROM 
						sv_client_sites c 
			WHERE 
						c.client_id = {$sv->clientId}";
	$core->db->query($sql);
// 	$core->db->select()->from('sv_client_sites')->fields('site_id id', 'name')->where('client_id = '.$sv->clientId);
// 	$core->db->execute();
	$core->db->get_rows(false,'id');
	$core->tpl->assign('sites_list', $core->db->rows);
	$core->tpl->assign('custom_ncity_user', $core->db->rows);
	
	if(!isset($_SESSION['current_catalog_site']) || !isset($core->db->rows[$_SESSION['current_catalog_site']])) {
		$_SESSION['current_catalog_site'] = key($core->db->rows);
		if(!$_SESSION['current_catalog_site']) {
			$_SESSION['current_catalog_site'] = 0;
			$core->tpl->assign('custom_ncity_user',1);
		}
	} else {
		if(!$_SESSION['current_catalog_site']) {
			unset($_SESSION['current_catalog_site']);	
		}
	}
}
...
$core->ajax->register($root); 
$core->ajax->register($core->tpl, 'get');
$core->ajax->listen();
$core->tpl->assign('current_site', $root->site_id);
