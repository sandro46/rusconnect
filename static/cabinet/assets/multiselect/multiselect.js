...
$.fn.wl_Multiselect = function (method) {
	...
	return this.each(function () {
		...
		if (!$this.data('wl_Multiselect')) {
			...
			var tpl = '<div class="comboselectbox">'+ 
						'<div class="combowrap">'+
							'<ul class="comboselect"></ul>'+
						'</div>'+
						'<div class="comboselectbuttons">'+
					  		//'<a class="add bnt"><i class="icon-step-forward"></i></a>'+
					  		'<a class="addall btn"><i class="icon-forward"></i></a>'+
					  		'<a class="removeall btn"><i class="icon-backward"></i></a>'+
					  		//'<a class="remove bnt"><i class="icon-step-backward"></i></a>'+
					  	'</div>'+
					  	'<div class="combowrap">'+
					  		'<ul class="comboselect"></ul>'+
					  	'</div>'+
		...
		$.each($this.find('option'), function (i, e) {
			...
			if (_this.is(':selected')) {
				...
				item.clone(true).data({
					'pos': j++
				}).attr('data-value', value).append('<a class="remove"><i class="icon-trash"></i></a>').appendTo(opts.selection).find('.add').remove();
...
$.fn.wl_Multiselect.methods = {
	...
	select: function (values) {
		...
		$.each(values, function (i, value) {
			...
			item.removeClass('selected').clone(true).attr('data-value', value).append('<a class="remove"><i class="icon-trash"></i></a>').appendTo(opts.selection).find('.add').remove();
			item.data('native').prop('selected', true);
			item.addClass('used');
			if (!opts.showUsed) item.hide();
		});
