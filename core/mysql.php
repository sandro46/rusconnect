...
class mysql_ext
{
	...
	public function filter($columnName, $callable) {
	    $this->filters[$columnName] = $callable;
	}
		...
		if($one_row) {
			...
			if($row) {
				...
				if(!empty($this->filters)) {
				    foreach($this->filters as $columnName=>$callable) {
				        if(isset($_row[$columnName])) {
				            $_row[$columnName] = call_user_func_array($callable, array($_row[$columnName], $_row, &$_row, $columnName));
				        }
				    }
				}
			}
			...
			$this->rows = (!isset($ret) || !is_array($ret))? array() : $ret;
