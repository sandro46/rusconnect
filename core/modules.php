...
class module {
	...
	public function module() {
		...
		$this->filters['product_url'] = function($field, $row) {
		    $id = (!isset($row['id']))? $row['product_id'] : $row['id'];
		    return (empty($row['rewrite']) || $row['rewrite'] == '')? '/' . core::$instance->lang . sprintf('/shop/product/id/%s/', $id) : $row['rewrite'];
		};
		
		$this->filters['category_url'] = function($field, $row) {
		    $id = (!isset($row['id']))? $row['group_id'] : $row['id'];
		    return (!$row['rewrite'])? '/' . core::$instance->lang . sprintf('/shop/category/id/%s/', $id) : $row['rewrite'];
		};
		
		$this->filters['page_url'] = function($field, $row) {
		    return (!$row['rewrite'])? '/' . core::$instance->lang . sprintf('/pages/show/id/%s/', $row['id']) : $row['rewrite'];
		};
		
		$this->filters['price'] = function($field, $row) {		
			if(isset($row['sales']) && intval($row['sales']) == 2) {
				if($row['price'] == $row['old_price']) {
					return parse_rub($row['price']-$row['sales_summ']);
				}
			}			
		    return parse_rub($field);
		};
		
		$this->filters['price_new'] = function($field, $row, &$link, $colname) {
		    $sourcePrice = ($row['price_type'] == 1)? $field : $field / floatval($row['pack_size']);
		    
		    if($row['min_order'] == 2) {
		        if($row['price_type'] == 1) {
		            $price = $field;
		        } else if($row['price_type'] == 2) {
		            $price = floatval($field) / floatval($row['pack_size']);
		        }
		    } else if($row['min_order'] == 1) {
		        if($row['price_type'] == 1) {
		            $price = floatval($field) * floatval($row['pack_size']);
		        } else if($row['price_type'] == 2) {
		            $price = $field;
		    ...
		    $price = round($price, 2);
		    $sourcePrice = round($sourcePrice, 2);
		    
		    $link[$colname.'_f'] = $price;
		    $link[$colname.'_s'] = parse_rub($sourcePrice);
		    $link[$colname.'_sf'] = $sourcePrice;
		    
		    
		    return parse_rub($price);
		};
		
		$this->filters['datetime'] = function($field, $row) {
		    return date('d-m-Y - H:i',$field);
		};
				
		$this->filters['date'] = function($field, $row) {
