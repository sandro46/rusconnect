...
class main_module extends module {
	...
	public function init() {
		...
		if($this->clientId <= 2) {
		    ...
		    if(!self::$loaded && MAINTENANCE ) {
		        ...
		        $this->tpl->assign('content', $this->tpl->get('maintenances.html', 'Cabinet'));
		        $this->core->module_name = ($this->core->is_admin())? $this->core->getAdminModule() : $this->core->site_name;
		        $this->tpl->assign('title', 'Технические работы.');
		        $this->tpl->display($this->core->main_tpl);
		        die();
		    }
        ...
        if(!empty($notices)) {
            ...
            foreach($notices as $item) {
                $query = array(
                    ...
                    'message_setting'=>mysql::str(json_encode(json_decode($item['data'],true))),
                ...
                $client->doBackground("process:rc:notice", $messageId);
	...
	protected function loadClientInfo($byClientId = false, $fromClientInterface = false) {
		if(!$byClientId) {
			$sql = "SELECT
						u.user_id,
						u.enabled as user_active,
...
class global_client_api extends main_module {
	...
	public function init() {
	    if(self::$loaded) {
	        $this->site = self::$instance['site'];
	        $this->shopId = self::$instance['shopId'];
	        $this->clientId = self::$instance['clientId'];
	        $this->shopInfo = self::$instance['shopInfo'];
		...
		$this->loadClientInfo($this->clientId, true);
		$this->tpl->assign('shopInfo', $this->shopInfo);
