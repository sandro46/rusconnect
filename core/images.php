...
class images
{
	...
	public function rotate($degre) {
	    ...
	    if($this->mime == 'image/png') {
	        ...
	        $rotation = imagerotate($this->source, $degre, imageColorAllocateAlpha($this->source, 0, 0, 0, 127));
	        imagealphablending($rotation, false);
	        imagesavealpha($rotation, true);
	    } else {
	        $rotation = imagerotate($this->source, $degre, imagecolorallocate($this->source, 255,255,255));
	    }
	...
	public function crop($x, $y, $width, $height) {
	    ...
	    $width = ($width > $this->info['width'])? $this->info['width'] : $width;
	    $height = ($height > $this->info['height'])? $this->info['height'] : $height;
		    
	    $x = ($x < 0)? 0 : $x;
	    $y = ($y < 0)? 0 : $y;

	    if($x + $tmp1 > $this->info['width']) {
	        $width = $this->info['width'] - $x;
	    }
	     
	    if($y + $tmp2 > $this->info['height']) {
	        $height = $this->info['height'] - $y;
	    }
	    
	    //var_dump(array($tmp1,$tmp2, $destimage, $this->source,$shiftx,$shifty,$x,$y, $width, $height, $sourcew, $sourceh));
	    $result = imagecopyresampled($destimage, $this->source,$shiftx,$shifty,$x,$y, $width, $height, $width, $height);
	    if(!$result) {
