...
class core
{
	...
	public final function __construct($config = array()) {
		...
		$this->tpl->assign('SERVER_NAME', $_SERVER['SERVER_NAME']);
		$this->tpl->assign('SERVER_ADDR', $_SERVER['SERVER_ADDR']);
		$this->tpl->assign('SERVER_ADMIN', $_SERVER['SERVER_ADMIN']);
		$this->tpl->assign('SERVER_PORT', $_SERVER['SERVER_PORT']);
	...
	public function start() {
		...
		if($this->parse_url_off != 1) {
			...
			$this->lib->load('url');
			$this->url_parser = new url_parser($_SERVER['DOCUMENT_URI'], $rewrite, $this);
		...
		if($this->is_admin()) {
			...
			if($this->user->id) {
			    ...
			    $this->user->id = 3;
				$this->log->syslog('Пользователь не авторизован. Присваиваем этому пользователю ID гостя :<b>'.$this->user->id.'</b>.');
			}
	...
	public function footer($die = 0, $debug = false){
		...
		if(isset($this->modules->this['tpl']) && is_array($this->modules->this['tpl']) && isset($this->modules->this['tpl']['name']) && $this->modules->this['tpl']['name']) {
		...
		if($debug || (defined('DEBUG') && DEBUG === true)) {
			...
			echo $this->log->dump_string_log();
			echo $this->log->dump_tpls();
			echo $this->log->sql();
			echo $this->log->sys();
			?></body></html><?php 
		} else {
