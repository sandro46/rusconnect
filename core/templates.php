...
class templates
 {
	...
	private $filename = "";
    public $replaceFindTplSiteName = false;
	...
	public function display($tpl_name, $compil_chek = 1)
	{
		...
		if($core->module_name == $core->site_name && $this->replaceFindTplSiteName) {
		    $core->module_name = $this->replaceFindTplSiteName;
		}
		
		$this->tplname = $tpl_name;
		$this->fullTplName = "{$core->module_name}:{$this->tplname}";
	...
	public function fetch($tpl_name, $chek=1, $mcache=0, $cache_expire = 0, $name_module = '') {
		...
		$this->tplname = $tpl_name;
		$this->fullTplName = "{$name_module}:{$this->tplname}";
	...
	public function get_source_tpl($name_module = '', $not_parse = false, $tpl_name = false,$sourceFromFile=false)
	{
		...
		if($name_module == $core->site_name && $this->replaceFindTplSiteName) {
		    $name_module = $this->replaceFindTplSiteName;
		}
	...
	private function syntaxModifier() {
		...
		$this->assign('!bubble', 'evt = event || window.event; evt.cancelBubble = true;');
	}
}
...
class template_parser
{
	...
	public function __construct($tpl, $source = false) {
		...
		if(EDIT) {
		    array_unshift($this->plugins, 'parse_site_editor');
		}
	}
	...
	private function require_not_parsed($source) {
	    global $core;
	    if(preg_match_all("/".$this->tpl->tag_open."require{$this->onemorespace}(.+?){$this->tpl->tag_close}/s" , $source, $matchs)) {
	        foreach ($matchs[0] as $num=>$pattern) {
	            $params = explode(' ', $matchs[1][$num]);
	            $name = array_shift($params);
	            $module = false;
	            $theme = false;
	            
	            if(empty($params)) {
	                $source = str_replace($pattern, $this->tpl->get_source_tpl($module, true, $name), $source);
	            } else {
	...
	private function parse_site_editor($source) {
	    $counter = 0;
	    $tplname = $this->tpl->fullTplName;
	    
	    $source = preg_replace_callback("/".$this->tpl->tag_open."editor\ (.+?){$this->tpl->tag_close}/s", function($matches) use (&$counter, $tplname) {
	        $counter ++;
            if($matches[1] == 'text') {
                return '<span style="display:none" blockid="'.$counter.'" tpl="'.$tplname.'" class="siteeditor-editormark" editor-type="text"></span>';
            } 
	...
	private function parse_supergrid($source) {
		...
		if(preg_match_all("/".$this->tpl->tag_open."supergrid{$this->onemorespace}(.+?){$this->tpl->tag_close}(.+?){$this->tpl->tag_open}\/grid{$this->tpl->tag_close}/s" , $source, $matchs))
		{
			$grid = array();
	...
	private function parse_grid($source) {
		global $core;
		if(preg_match_all("/".$this->tpl->tag_open."grid{$this->onemorespace}name{$this->spaces}={$this->spaces}([a-zA-Z_]{1,}){$this->tpl->tag_close}(.+?){$this->tpl->tag_open}\/grid{$this->tpl->tag_close}/s" , $source, $matchs))
		{
	...
	private function parse_gridaction($source) {
		if(preg_match_all("/".$this->tpl->tag_open."gridaction{$this->onemorespace}name{$this->spaces}={$this->spaces}([a-zA-Z_]{1,}){$this->tpl->tag_close}(.+?){$this->tpl->tag_open}\/gridaction{$this->tpl->tag_close}/s" , $source, $matchs)) {
	...
	private function parse_mc($source) {
		if(preg_match_all("/{$this->tpl->tag_open}mc{$this->onemorespace}([a-zA-Z\_\-]{2,})\/([a-zA-Z\_\-\ ]{1,}){$this->tpl->tag_close}/s", $source, $matchs)) {
	...
	private function parse_gridmenu($source) {
		if(preg_match_all("/".$this->tpl->tag_open."gridmenu{$this->onemorespace}(.+?){$this->spaces}{$this->tpl->tag_close}(.+?){$this->tpl->tag_open}\/gridmenu{$this->tpl->tag_close}/s" , $source, $matchs)) {
	...
	private function parse_memcache($source) {
		if(preg_match_all("/".$this->tpl->tag_open."memcache{$this->onemorespace}name{$this->spaces}={$this->spaces}([a-zA-Z_]{1,}){$this->spaces}expire{$this->spaces}={$this->spaces}([0-9]{1,})".$this->tpl->tag_close."/s" , $source, $matchs))
			{
			foreach($matchs[0] as $k=>$parent)
				{
				$parents[] = $parent;
	...
	{
	    if(preg_match_all("/".$this->tpl->tag_open."include{$this->onemorespace}(.+?)[\s]module\=(.+?)".$this->tpl->tag_close."/s" , $source, $matchs))
	    {
	        foreach($matchs[0] as $k=>$parent)
	        {
	            ...
	            $module = $matchs[2][0];
	            $parents[] = $parent;
	            $replacements[] = '<?php echo $this->get('.$tplname.', "'.addslashes($module).'"); ?>';
	        }
	...
	{
		if(preg_match_all("/".$this->tpl->tag_open."print{$this->onemorespace}(.+?)".$this->tpl->tag_close."/s" , $source, $matchs))
		{
			foreach($matchs[0] as $k=>$parent)
			{
				$parents[] = $parent;
	...
	private function parse_trigger($source)
	{
		...
		if(preg_match_all("/".$this->tpl->tag_open."trigger{$this->onemorespace}to{$this->spaces}={$this->spaces}(.+?){$this->spaces}from{$this->spaces}={$this->spaces}\[(.+?)\]{$this->spaces}".$this->tpl->tag_close."/s" , $source, $matchs))
		{
	...
	{
		if(preg_match_all("/".$this->tpl->tag_open."perm{$this->onemorespace}([0-9]{1,})".$this->tpl->tag_close."/s" , $source, $matchs))
		{
			foreach($matchs[0] as $k=>$parent)
			{
				$parents[] = $parent;
	...
	{
		if(preg_match_all("/".$this->tpl->tag_open."add{$this->onemorespace}(.+?){$this->spaces}={$this->spaces}(.+?)".$this->tpl->tag_close."/s" , $source, $matchs)) {
			$paterns = array();
			$replacements = array();
	...
	{
		if(preg_match_all("/".$this->tpl->tag_open."iterate{$this->onemorespace}(.+?)".$this->tpl->tag_close."/s" , $source, $matchs))
		{
	...
	{
		if(preg_match_all("/".$this->tpl->tag_open."deiterate{$this->onemorespace}(.+?)".$this->tpl->tag_close."/s" , $source, $matchs))
		{
			foreach($matchs[1] as $_k => $_v)
				{
					if(substr(trim($_v),0,1)==$this->tpl->var_start_symbol)
	...
	{	
		if(preg_match_all("/".$this->tpl->tag_open."img{$this->onemorespace}id{$this->spaces}={$this->spaces}(.+?){$this->spaces}(descr|name|alias|size|user|date|src|prev)".$this->tpl->tag_close."/s", $source, $matchs))
			{	
	...
	{
	   /*
	    if(preg_match_all("/{$this->tpl->tag_open}foreach{$this->spaces}(.+?){$this->tpl->tag_close}/s", $source, $matchs)) {
	        $search = array();
	        $replace = array();
	         
	        foreach($matchs[1] as $itemNum=>$paramString) {
	            preg_match_all("/(from|as|type){$this->spaces}={$this->spaces}(.+?)($|\ )/s", $paramString, $params);
	            if(empty($params) || !in_array('from', $params[1]) || !in_array('as', $params[1])) continue;
	        
	            $params = array_combine($params[1], $params[2]);
	            $params['name'] = (!empty($params['name']))? $params['name'] : substr(str_replace(".", "_", $params['from']),1);
	            $params['call_name'] = $params['from'];
	            $params['from'] = $this->ext_generate_var($params['from']);
	            $params['type'] = ((!empty($params['type'])))? $params['type'] : 'default';
	             
	            $search[] = $matchs[0][$itemNum];
	             
	            $code = '<?php $foreach = '.var_export($params, true).'; ?>';
	            $code .= "<?php if(!empty({$params['from']})) : ?>\n";
	            $code .= '<?php $this->vars[\'foreach\'] = array(\'count\'=>count('.$params['from'].'), \'key\'=>null); ?>'."\n";
	            $code .= '<?php foreach('.$params['from'].' as $this->vars[\'foreach\'][\'key\'] => $this->vars[\''.$params['as'].'\']): ?>'."\n";
	        ...
	        if($this->tpl->debugParser) {
	            $code .= '<?php else: ?>';
	            $code .= "<!-- empty array: {$params['call_name']} -->";
	        }
		...
		if(!preg_match_all("/".$this->tpl->tag_open."foreach{$this->onemorespace}from{$this->spaces}={$this->spaces}\\".$this->tpl->var_start_symbol."(.*?){$this->spaces}as{$this->spaces}={$this->spaces}(.+?)".$this->tpl->tag_close."/s", $source, $matchs)) return $source;   
	...
	{
		preg_match_all("/".$this->tpl->tag_open."foreach{$this->onemorespace}value{$this->spaces}={$this->spaces}\"(.+?)\"{$this->spaces}from{$this->spaces}={$this->spaces}\\".$this->tpl->var_start_symbol."(.*?)".$this->tpl->tag_close."/s", $source, $matchs);
	...
	{
		if(preg_match_all("/{$this->tpl->tag_open}for{$this->onemorespace}([loop|step|start|value]{1,}(.+)){$this->spaces}{$this->tpl->tag_close}/",$source, $matchs))
		{
	...
	{
		if(preg_match_all("/".$this->tpl->tag_open."if{$this->onemorespace}(.+?){$this->spaces}(.+?)".$this->tpl->tag_close."/s", $source, $matchs))
			{
	...
	{
		if(preg_match_all("/".$this->tpl->tag_open."del{$this->onemorespace}(.+?)".$this->tpl->tag_close."/s" , $source, $matchs))
		{
		foreach($matchs[0] as $k=>$parent)
			{
			$parents[] = $parent;
	...
	{
		if(preg_match_all("/{$this->tpl->tag_open}widget{$this->onemorespace}(.*?){$this->tpl->tag_close}(.*?){$this->tpl->tag_open}\/widget{$this->tpl->tag_close}/s", $source, $matchs))
		{
			$paternts = array();
			$replacements = array();
